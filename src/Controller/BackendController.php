...
class BackendController extends Controller
{
    ...
    public function addImages(Project $project, Request $request, ImageCompressor $compressor){
        ...
        if ($artworkForm->isSubmitted() && $artworkForm->isValid()) {
            ...
            foreach($files as $file){
                ...
                $file->move(
                    $this->getParameter('artwork_images'),
                    $fileName
                );

                $compressor->resizeImage($this->getParameter('artwork_images'), $fileName);
                $thumbnailName = $compressor->createThumbnail($this->getParameter('artwork_images'), $fileName);
    ...
    public function removeArtwork(Artwork $artwork){
        ...
        $entityManager = $this->getDoctrine()->getManager();
        if($project->getFrontArt() == $artwork)
            $project->setFrontArt(null);
    ...
    public function deleteImages($artworks){
        foreach ($artworks as $artwork){
            if (file_exists($this->getParameter('artwork_images') . $artwork->getImage()))
                unlink($this->getParameter('artwork_images') . $artwork->getImage());
            if (file_exists($this->getParameter('artwork_images') . $artwork->getThumbnail()))
                unlink($this->getParameter('artwork_images') . $artwork->getThumbnail());
        }
    }
    ...
    public function removeProject(Project $project){
        $entityManager = $this->getDoctrine()->getManager();
        $project->setFrontArt();
        $this->deleteImages($project->getArtworks());
     ...
     */
    public function addProjectFrontImage(Artwork $artwork){
        $artwork->getProject()->setFrontArt($artwork);
...
//  Do not use these functions unless you have a good reason to
    ...
    private function resizeAllThumbnails($compressor, $projects){
        foreach ( $projects as $project)
            foreach ( $project->getArtworks() as $artwork)
                $compressor->resizeThumbnail($this->getParameter('artwork_images') . $artwork->getThumbnail());
    }
